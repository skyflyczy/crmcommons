...
public class MemCachedClient {
		...
		try {
			...
			if ( DELETED.equals( line ) ) {
				...
				if (log.isInfoEnabled())
					log.info( new StringBuffer().append("++++ deletion of key: ").append(key).append(" from cache was a success").toString() );
				...
				if (log.isInfoEnabled())
					log.info( new StringBuffer().append("++++ deletion of key: ").append(key).append(" from cache failed as the key was not found").toString());
			}
			else {
			...
			try {
				...
				log.error( new StringBuffer().append("++++ failed to close socket : ").append(sock.toString()).toString() );
			}
	...
	private boolean set( String cmdname, String key, Object value, Date expiry, Integer hashCode, boolean asString ) {
        ...
        if ( NativeHandler.isHandled( value ) ) {
			...
			if ( asString ) {
				...
				try {
					if (log.isInfoEnabled())
						log.info( new StringBuffer().append("++++ storing data as a string for key: ")
					...
					log.error( new StringBuffer().append("invalid encoding type used: ").append(defaultEncoding).toString(), ue );
					sock.close();
					sock = null;
					return false;
				}
			...
			try {
				if (log.isInfoEnabled())
					log.info( new StringBuffer().append("++++ serializing for key: ")
							.append(key).append(" for class: ").append(value.getClass().getName()).toString() );
		...
		if ( compressEnable && val.length > compressThreshold ) {
			...
			try {
				...
				if (log.isInfoEnabled())
					log.info(new StringBuffer().append("++++ compression succeeded, size after: ").append(val.length).toString() );
			}
			catch ( IOException e ) {
		...
		try {
			...
			if (log.isInfoEnabled())
				log.info( new StringBuffer().append("++++ memcache cmd (result code): ").append(cmd)
						.append(" (").append(line).append(")").toString() );

			if ( STORED.equals( line ) ) {
				
				if (log.isInfoEnabled())
					log.info(new StringBuffer().append("++++ data successfully stored for key: ").append(key).toString() );
			...
			{
				if (log.isInfoEnabled())
					log.info( new StringBuffer().append("++++ data not stored in cache for key: ").append(key).toString() );
			}
			else {
				
				log.error( new StringBuffer().append("++++ error storing data in cache for key: ")
	...
	public long getCounter( String key, Integer hashCode ) {
		...
		try {
			...
			if (log.isInfoEnabled())
				log.info( new StringBuffer().append("Failed to parse Long value for key: ").append(key).toString() );
		}
	...
	public long addOrIncr( String key, long inc, Date expire, Integer hashCode ) {
		boolean ret = set( "add", key, new Long( inc ), expire, hashCode, true );
		if ( ret ) {
			return inc;
		}
		else {
		...
		try {
			...
			if ( line.matches( "\\d+" ) ) {
				...
				try {
					...
					log.error( new StringBuffer().append("Failed to parse Long value for key: ")
							.append(key).toString() );
				}
 			}
			else if ( NOTFOUND.equals( line ) ) {
	...
	public Map<String,Object> getMulti( String[] keys, Integer[] hashCodes, boolean asString ) {
		...
		if (log.isInfoEnabled())
			log.info( new StringBuffer().append("multi get socket count : ").append(cmdMap.size()).toString() );
	...
	protected final class NIOLoader {
		...
		private final class Connection {
			...
			public String toString() {
				return new StringBuffer().append("Connection to ").append(sock.getHost())
		...
		public void doMulti( boolean asString, Map<String, StringBuilder> sockKeys, String[] keys, Map<String, Object> ret ) {
			...
			try {
				...
				while ( numConns > 0 && timeRemaining > 0 ) {
					int n = selector.select( Math.min( timeout,  10000 ) );
					if ( n > 0 ) {
