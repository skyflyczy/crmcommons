...
public class XMsgError extends XMsgBase{
	...
	public String toMessage(boolean isDebugEnabled){
		...
		if(exception instanceof ErrorCodeException) {
			ErrorCodeException be = (ErrorCodeException)exception;
    		sb.append(_SepChar_Start).append(exception.getClass().getName()).append(":").append(be.getErrorCode().getCode()).append(",").append(be.getMessage()).append(_SepChar_End);
    	}else {
